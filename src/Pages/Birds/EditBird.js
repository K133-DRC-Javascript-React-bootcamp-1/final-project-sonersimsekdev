...
export default function EditBird({ name_ , food_ , weight_, health_, age_ , img_}) {
  ...
  const [food, setFood] = useState(food_);
  const [weight, setWeight] = useState(weight_);
  const [health, setHealth] = useState(health_);
  const [age, setAge] = useState(age_);
  const [img, setImg] = useState(img_);
